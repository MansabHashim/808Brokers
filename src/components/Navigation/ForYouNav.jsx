...
const ForYouNav = () => {
  ...
  const isActive = (path) => {
    if (path === '/foryou' && (currentPath === '/foryou' || currentPath === '/')) {
      return true;
    }
  ...
  const [storageUsed, setStorageUsed] = useState(0.002); // Starting with 2MB
  const storageLimit = 1; // 1GB limit

  // Simulated storage update (replace this with actual Google Cloud API call)
  useEffect(() => {
    const fetchStorageData = async () => {
      try {
        // This will be replaced with actual Google Cloud API call
        ...
        const mockUsage = Math.random() * 0.01; // Random usage between 0-10MB
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Link to="/foryou" className={getLinkClass('/foryou')}>
